...
!0}catch(f){return console.error(f),!1}}function recalcUnsavedChanges(){if(null===lastSavedSchedules)return!1;var d=currentSchedulesToJSON();document.body.classList.toggle("schedulesExist",!!d.length);if(usingURL)return!1;d=JSON.stringify(d)!==JSON.stringify(lastSavedSchedules);unsavedChanges.className=d?"":"hidden";return d}function saveState(d){setUsingURL(!1);lastSavedSchedules=currentSchedulesToJSON();unsavedChanges.className="";localforage.setItem("savedSchedules",lastSavedSchedules,d)}
